...
export function registerApiKeyRoutes(app: FastifyInstance): void {
  ...
  app.post('/api/v1/apikey', async (request: FastifyRequest, reply) => {
    const body = request.body as ApiKeyBody | null
    ...
    const keys = createApiKey(body.keyName)
    return reply.code(200).send(keys)
  })

  app.put('/api/v1/apikey/:id', async (request: FastifyRequest<{ Params: IdParams }>, reply) => {
